...
test("DemoBlaze", async ({ page }) => {
  ...
  await home.addProductToCart("Samsung galaxy s6", "MacBook Pro", "Apple monitor 24");
  ...
  await page.waitForTimeout(5000);
  const status = await cart.checkProductInCart("Samsung galaxy s6", "MacBook Pro", "Apple monitor 24");
  expect(await status).toBe(true);

  //PalceOrder
  const order = new PlaceOrder(page);
  await order.shippingAddress("Kriash", "Dubaie", "AbuDabi", "1750125369424589", "Feb", "2024");
  await page.waitForTimeout(5000);
